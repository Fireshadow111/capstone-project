<template>
  <body>
    <navbar/>

    <!-- Display spinner when products are not loaded -->
    <spinner v-if="!products"></spinner>

    <div v-if="products" class="container text-center">
      <div class="row align-items-center">
        <div id="prod-head" class="mt-5">
          <h1 id="prodDrag">Products</h1>
        </div>
      </div>
    </div>

    <div v-if="products" class="container">
      <div class="row justify-content-center text-center">
        <div class="col-md-6 mt-4">
          <form class="d-flex" role="search">
            <input id="products-search-input" class="form-control me-2" data-searchBar type="search"
              placeholder="Search..." aria-label="Search" style="width: 600px;">
            <button id="products-search-button" class="btn btn-outline-success" type="submit" data-searchBtn>
              Search
            </button>
          </form>
        </div>

        <div class="col-md-6 mt-4">
          <div class="d-flex" role="search">
            <button id="products-sort-button" class="btn btn-outline-success" data-sortBtn>Sort</button>
            <input id="products-sort-input" class="form-control me-2" type="search" disabled
              placeholder="Lowest to Highest..." aria-label="Search" style="width: 600px;">
          </div>
        </div>

        <div class="col-md-4 mt-5" v-for="item in products" :key="item">
          <div class="card homeCard">
            <img :src="item.prod_URL" alt="" class="card-img-top w-100" style="height: 300px;">
            <div class="card-body" style="background-color:white;">
              <h3 style="color:#e10800; font-weight: bold; background-color: black;" class="card-title mb-4">
                {{ item.prod_Name }}
              </h3>
              <div>
                <div class="d-flex justify-content-between deats">
                  <router-link :to="{name: 'product', params: {id:item.prod_ID}}">
                    <button type="button" class="btn btn-sm view" style=" font-weight: bold;">View More</button>
                  </router-link>
                  <p style="color: black;" class="my-1 price">${{ item.price }}</p>
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>
    </div>

    <footer id="footer-con" class="container py-3 my-5" style="font-family: fantasy;">
      <p id="footer" class="text-center text">
        Redragon - Copyright© - 2024 | All Rights Reserved
      </p>
    </footer>
  </body>
</template>

<script>
import spinner from '../components/spinner.vue'
import navbar from '../components/navbar.vue'
import footer from '../components/footer.vue'

export default {
  name: 'HomeView',
  components: {
    navbar,
    footer,
    spinner
  },
  data() {
    return {
      prod_URL: null,
      prod_Name: null,
      category: null,
      price: null
    }
  },
  methods: {},
  computed: {
    products() {
      return this.$store.state.products
    },
    getProducts() {
      return this.$store.dispatch('getProducts')
    },
  },
  mounted() {
    this.getProducts
  },
}
</script>
<style scoped>

body{
   background-color: black;
 
}

.prodHead{
  text-align: center;
}


.homeCard{
  border-radius: 0px;
  height: 420px; 
  max-width: 400px;
  background-color: black;


  box-shadow: 0 0 25px #e10800;
    transition: transform 0.3s ease, box-shadow 0.3s ease;

}

.homeCard:hover{
  transform:scale(105%);
    box-shadow: 0 0 40px #e10800;
}

.col{
  justify-content: center;
}

.card img{
  max-width: 510px;
}

.deats{
  align-items: center;
}

.view{
  border-radius: 0px;
  border: 2px solid black;
  color: black;

  
}

.view:hover{
  
  background-color: black;
  color:#e10800;

}

.view:active{
  transform: translatex(4px);
 
}

.card-title{
  font-size: 23px;
}
.price{
  font-size: 19px;
  font-weight: bold;
  color: white
}



@media screen and (width <= 375px) {
  .sneaks{
  min-width: 300px;
}
}

#footer-con{
  border-top: 2px solid #e10800;
}

#footer{
color: #e10800;
}


#prodDrag{

  color: #e10800;
  font-weight: bolder;
font-family: "Montserrat";
  font-size: 60px;

}



#prod-head {
  background-color: black;
  color: white;
  border: 4px solid  #e10800;
  border-radius: 55px;
  font-family: "DM Serif Display";
  font-size: 50px;



}

#products-search-button {
  width: 150px;
  height: 50px;
  color: black;
  font-family: "Roboto";
  font-size: 18px;
  font-weight: bold;
  background-color: #e10800;
  border: solid 3px black;
  border-radius: 50px;
  cursor: pointer;
  transition: border-color 0.5s, background-color 0.5s, color 0.5s;
}

#products-search-button:hover {
  background-color: black;
  color: #e10800;
  border-color: white;
}

#products-search-button:active {
  box-shadow: 0 5px #666;
  transform: translatex(4px);
}

#products-sort-button {
  width: 150px;
  height: 50px;
  color: black;
  font-family: "Roboto";
  font-size: 18px;
  font-weight: bold;
  background-color:#e10800;
  border: solid 3px black;
  border-radius: 50px;
  cursor: pointer;
  transition: border-color 0.5s, background-color 0.5s, color 0.5s;
}

#products-sort-button:hover {
  background-color: black;
  color: #e10800;
  border-color: white;
}

#products-sort-button:active {
  box-shadow: 0 5px #666;
  transform: translatex(4px);
}




@media screen and (max-width: 1080px) {
  .homeCard {
    max-width: 300px; 
    height: auto; 
    margin: auto
  }

  .card img {
    max-width: 100%; 
    height: auto; 
  }
}



@media screen and (max-width: 720px) {
  .homeCard {
    max-width: 250px; 
    height: auto; 
    margin: auto
  }

  .card img {
    max-width: 100%; 
    height: auto; 
  }

  #prod-head {
    font-size: 25px; 
  }
}


@media screen and (max-width: 300px) {
  .homeCard {
    max-width: 200px; 
    height: auto;
    margin: auto
  }

  .card img {
    max-width: 100%;
    height: auto; 
  }

  #prod-head {
    font-size: 20px; 
  }
}


</style>